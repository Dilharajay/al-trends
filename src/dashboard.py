...
if page == "Overview":
    ...
    if not d_valid.empty:
        ...
        st.subheader("Highest Cutoffs by University")
        top_unis = d_valid.groupby("UniversityName")["CutoffZ"].max().sort_values(ascending=False).reset_index()
        fig_bar_uni = px.bar(top_unis, x="CutoffZ", y="UniversityName", orientation='h', title="Highest Cutoffs by University")
        fig_bar_uni.update_layout(yaxis={'categoryorder':'total ascending'})
        st.plotly_chart(fig_bar_uni, width='stretch')
    ...
    ### Analytical Metrics
    - **Cutoff Z-Score**: The base metric representing the official UGC published minimum Z-score threshold for a specific course and district combination in a given year.
    - **Cutoff Volatility**: Calculated as the standard deviation of a course's Z-score cutoff across all available historical years. It indicates how much the cutoff fluctuates year-over-year. It is binned into three qualitative categories: *Low*, *Medium*, and *High*.
    - **Competitiveness Index (0-100)**: A custom composite metric designed to rank courses. It normalizes the historical mean Z-score against the national minimum and maximum, and penalizes high volatility. A score closer to 100 indicates a historically difficult course to enter with very stable high cutoffs. *Note: This is an exploratory indicator, not an official UGC measure.*
    - **Competitiveness Band**: A qualitative grouping of the Competitiveness Index into five tiers: *Lower historical cutoff*, *Moderate*, *Competitive*, *Highly competitive*, and *Extremely competitive*.
    - **Year-over-Year (YoY) Change**: Measures the absolute point difference and percentage change between the cutoff of the most recent academic year and the immediately preceding year.
    - **Student Margin**: Used in the Student Analyzer, this is the simple arithmetic difference between a hypothetical student's inputted Z-score and the historical cutoff for that district. 
    - **Eligibility Likelihood**: Categorizes the Student Margin into four bands (*Safe*, *Possible*, *Competitive*, and *Unlikely*) to estimate how comfortably a student would have cleared past cutoffs.
